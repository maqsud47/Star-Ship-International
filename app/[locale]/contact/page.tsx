...
export default async function ContactPage({
  ...
  // JSON-LD LocalBusiness for each office — strong signal for local SEO.
  const jsonLd = {
        ...
        <Container>
          ...
          <Reveal stagger className="mt-8 grid gap-6 lg:grid-cols-3">
            {OFFICES.map((o) => (
              <div
                key={o.key}
                className="ring-border shadow-card ssi-lift flex flex-col rounded-xl bg-white p-6 ring-1"
              >
                <div className="flex items-center gap-3">
                  <span className="bg-signal/10 text-signal inline-flex size-10 items-center justify-center rounded-lg">
                ...
                <div className="border-border mt-5 space-y-2 border-t pt-4 text-sm">
                  {o.phones.map((p) => (
                    <a
                      key={p}
                      href={telHref(p)}
                      className="text-slate/85 hover:text-signal flex items-center gap-2 transition-colors"
                    >
                  ...
                  {o.emails.map((e) => (
                    <a
                      key={e}
                      href={`mailto:${e}`}
                      className="text-slate/85 hover:text-signal flex items-center gap-2 break-all transition-colors"
                    >
          ...
          <aside className="flex flex-col gap-6">
            <div className="bg-navy rounded-2xl p-7 text-white">
              ...
              <Link
                ...
                className={cn(
                  buttonVariants(),
                  "mt-5 bg-signal text-white hover:bg-sky"
                )}
              >
            ...
            <div className="ring-border shadow-card rounded-2xl bg-white p-6 ring-1">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-navy font-heading text-sm font-semibold">
                    {t("cta.whatsapp")}
                  </div>
              ...
              <div className="border-border mt-4 space-y-2 border-t pt-4">
                {CONTACT.whatsapps.map((w) => (
                  <a
                    ...
                    className="text-slate/85 hover:text-signal block text-sm transition-colors"
                  >
                    {w.display}
                  </a>
                ))}
