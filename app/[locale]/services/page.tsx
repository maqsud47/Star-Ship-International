  ...
  return (
    <div className="grid items-center gap-8 lg:grid-cols-2 lg:gap-14">
      <Reveal className={cn(flip && "lg:order-2")}>
        ...
        <h2 className="text-h2 text-navy mt-5">{t(`services.${service.key}.name`)}</h2>
        <p className="text-signal mt-2 font-medium">
...
export default async function ServicesPage({
  ...
  return (
    <>
      ...
      <section className="border-border bg-cloud border-y py-16 sm:py-20">
        <Container>
          <Reveal>
          ...
          <Reveal stagger className="mt-8 grid gap-4 sm:grid-cols-2">
            {(t.raw("servicesPage.additional") as string[]).map((item) => (
              <div
                key={item}
                className="ring-border shadow-card ssi-lift flex items-center gap-3 rounded-xl bg-white px-5 py-4 ring-1"
              >
                <Boxes className="text-signal size-5 shrink-0" />
